...
From **0.1.54**, the web UI **reopens the chat you left open** the next time it loads, for example after the Home Assistant app on a phone reloads the panel. Choosing **New chat** is remembered as well. The choice is kept in the browser; the worker does not record which chat you had open.
...
From **0.1.50**, **pin, rename, and delete chats** from the sidebar. On desktop, hover over a chat and use its **⋯** button, or right-click the row; on phones, long-press the chat to open a bottom sheet. Pinned chats stay at the top under a **Pinned** heading, renamed titles show in the sidebar and the conversation header, and deleting asks for confirmation before removing the conversation, its saved history, and its Codex session. Running chats must be stopped before they can be deleted.

From **0.1.51**, **attach images to a message**. Use the paperclip button next to the model pill, paste a screenshot into the message box, or drop image files onto the composer, then send. Codex sees the images with your message, so you can show it a broken dashboard card, a device, or a design to build. Attached images appear with your message in the chat and stay with the conversation. PNG, JPEG, GIF, and WebP are supported, up to 6 images per message and 10 MB each.
...
![Desktop chat UI showing an image generated by Codex under its response, with a download link](examples/chat-ui/generated-image.png)
